...
{#if suggestion.suggestedTitle && !titleDismissed}
  <div class="mt-2 flex items-center gap-2 px-2 py-1.5 rounded-md bg-accent-violet/10 border border-accent-violet/20 text-[0.6875rem]">
    <span class="text-accent-violet">✦</span>
    <span class="flex-1 truncate font-medium">{suggestion.suggestedTitle}</span>
    <button class="px-2 py-0.5 rounded bg-accent-violet/30 text-accent-violet text-[0.625rem]" onclick={acceptTitle}>use</button>
    <button class="px-1.5 py-0.5 rounded bg-white/5 text-[0.625rem]" onclick={() => (titleDismissed = true)} aria-label="Dismiss">×</button>
  </div>
{/if}

{#if suggestion.suggestedCollectionPath && appliedCollectionId}
  <div class="mt-2 flex items-center gap-2 text-[0.6875rem]">
    ...
    <span class="opacity-40 text-[0.625rem] ml-auto">(change below)</span>
  </div>
{/if}
...
{#if suggestion.suggestedTags.length > 0}
  <div class="mt-2">
    <div class="flex flex-wrap gap-1">
      {#each suggestion.suggestedTags as t (t.name)}
        {#if !appliedTagNames.has(t.name) && !tagsManuallyAccepted.has(t.name)}
          <button
            class="px-2 py-0.5 rounded-full text-[0.625rem] border border-dashed"
    ...
    {#if suggestion.suggestedTags.some((t) => !appliedTagNames.has(t.name) && !tagsManuallyAccepted.has(t.name))}
      <div class="mt-1.5 text-[0.5625rem] opacity-40 flex gap-3">
